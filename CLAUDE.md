...
- Media uploads go to S3 in production (`@payloadcms/storage-s3`, workspace-minio, bucket `flatpackwholehome`) whenever `S3_BUCKET` is set; with no S3 env vars (local dev, Dockerfile build) they write to `public/media` on local disk. `public/media` in the production container is an anonymous Docker volume that a Coolify redeploy can recreate empty — that's what S3 fixes; don't reintroduce a local-disk-only media path in production. Do not add a Next rewrite from /api/media/file to /media: it bypasses Payload's storage handler and 404s every file once uploads live in S3. Regenerating `importMap.js` must be done with `S3_BUCKET` set (any value), otherwise the S3 upload component drops out of the map.
