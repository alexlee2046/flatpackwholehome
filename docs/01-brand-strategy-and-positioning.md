...
```mermaid
graph TD
    A["从【单品买卖】升级为【全屋解决方案】"] --> B["1. 客单价 (AOV) 成倍飙升<br/>从 $500 单张沙发 ➔ $1,500~$2,500 全屋入住套装 (Move-In Bundle)"]
    A --> C["2. 全屋设计语言统一 (Aesthetic Harmony)<br/>解决年轻人跑5家店家具颜色材质不搭的巨大痛点"]
    A --> D["3. DDP 专线物流协同效应<br/>所有产品均遵循平板/压缩包裹规范, 6个箱子装下一整套公寓"]
    A --> E["4. 通用 0 工具快装体验<br/>全系统一榫卯/金属自锁, 全屋 1 小时徒手拼完, 彻底告别螺丝刀"]
```
...
```mermaid
graph LR
    subgraph 五金架构 ["1. 通用免工具五金架构 (Hardware Engine)"]
        H1["【自锁滑槽卡扣】<br/>用于模块沙发与排骨架大件连接"]
        H2["【CNC 高精穿插榫卯】<br/>用于茶几、书架、板式家具 (0 螺丝)"]
        H3["【预装式折叠阻尼铰链】<br/>出厂已打好在板材上, 开箱直接拉开成型"]
    end

    subgraph 材料与配色 ["2. 全系统一设计美学 (Unified Material Palette)"]
        M1["【天然木作】<br/>FSC 认证天然白橡 / 桦木多层板"]
        M2["【金属骨架】<br/>哑光细砂黑 / 燕麦白粉末喷涂钢管"]
        M3["【面料软包】<br/>雪尼尔 / 羊羔绒 / 猫抓科技布 (6款全屋互通色)"]
    end

    五金架构 --> P["输出: 统一设计语言 + 极低装配门槛 + 零部件通用互换"]
    材料与配色 --> P
```
...
以 **$1,499.00** 的一室一厅 6 箱全套家具出海直发美线为例（按 **18 RMB/kg ($2.50/kg)** 高安全线全包核算）：

```
┌─────────────────────────────────────────────────────────────┐
│    一室一厅全屋套装 (6箱) 财务模型核算 (按 18 RMB/kg 测算)  │
├─────────────────────────────────────────────────────────────┤
│ 独立站全套零售价 (Bundle Retail Price, 包邮包税):   $1,499.00│
├─────────────────────────────────────────────────────────────┤
│ 1. 出厂全套制造成本 (BOM: 沙发+茶几+电视柜+大床+床头柜):-$335.00│
│ 2. 国际 DDP 海派到门 (102kg @ 18元/kg = ¥1,836元):    -$255.00│
├─────────────────────────────────────────────────────────────┤
│ ★ 履约贡献毛利 (Contribution Margin):                $909.00│
│ ★ 履约贡献毛利率:                                      60.6% │
├─────────────────────────────────────────────────────────────┤
│ 3. 独立站 CAC 获客成本 (Meta / Google 套装投流):    -$220.00│
│ 4. 支付网关与平台费 (Stripe 2.9% + Shopify 2%):      -$73.00│
│ 5. 备件支持、面料小样盒与以捐代退准备金 (3%):        -$45.00│
├─────────────────────────────────────────────────────────────┤
│ ★ 单套全屋纯利润 (Net Profit per Bundle):           $566.00│
│ ★ 综合纯净利润率 (Net Profit Margin):                 37.8% │
└─────────────────────────────────────────────────────────────┘
```

> **传统大件对比与抗风险裕度**：
> - 如果按照传统不可压缩大件家具发运一整套客厅+卧室，体积高达 **2.5 ~ 3.5 CBM**，末端必须使用超昂贵的卡车专线（Freight），DDP 运费将超过 **$800**；
> - 我们通过全平装压缩将运费压缩到 **$255**（按 18元/kg 保守估算），依然保留 **37.8% 净利润**；
> - 若后续出货量扩大或平季走以星/普船快线（11-13 元/kg），单套运费可进一步压至 **$178**，单套纯利将冲破 **$640+（净利率超 42%）**！
