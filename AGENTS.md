...
- Use `pnpm`. Start with the smallest relevant check, such as `pnpm exec eslint <changed-files>`; run full lint, build, or browser E2E only when the change warrants it.
- Routes and UI live in `src/app` and `src/components`; Payload schema lives in `src/collections`, `src/globals`, `src/blocks`, and `src/payload.config.ts`; locale behavior and catalogs live in `src/i18n`, `messages`, and `translations`.
- Do not hand-edit `.next/`, `public/media/`, `src/payload-types.ts`, or `src/app/(payload)/admin/importMap.js`. Regenerate tracked Payload outputs only after their source configuration changes.
